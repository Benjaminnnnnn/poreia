...
import { useTripsQuery } from "@/entities/trip/model/useTripsQuery";
import { useRefineTripMutation } from "@/entities/trip/model/useRefineTripMutation";
import { useUpdateTripItineraryMutation } from "@/entities/trip/model/useUpdateTripItineraryMutation";
...
function TripPageContent({ tripId }: { tripId: string }) {
  ...
  const { mutateAsync: refineTripAsync, isPending: isRefining } =
    useRefineTripMutation();
  const { mutate: updateTripItinerary } = useUpdateTripItineraryMutation();
  ...
  const handleManualItineraryUpdate = (newItinerary: TravelItinerary) => {
    updateTripItinerary({ tripId, itinerary: newItinerary });
  };
  ...
  const handleRefine = async (event: React.FormEvent) => {
    ...
    setInputValue("");
    await refineTripAsync({ tripId, prompt });
  };
